...
class TextCNN(nn.Module):
    ...
    def forward(self,x):
        ...
        #print(out[0].shape)
        out = torch.cat(out,dim=1) #[batch,300,1,1]
        #print(out.shape)
        out = out.view(in_size,-1) #将out变成(batch_size,output_num)
        out=self.fc(out)
        return out
...
def compute_classification(test_loader):
    ...
    model.load_state_dict(torch.load("record3/MTmodel.pth"))
    model.eval()
    ...
    result_name=open("result/MT150.txt",'a')
...
def train_textcnn_model(model,train_loader,val_loader,num_epoch,lr,lamda):
    ...
    for epoch in range(num_epoch):
        ...
        for i,data in enumerate(train_loader):
            ...
            batch_pred=model(data[0]) #预测的情感分布
    ...
    model_name=r'record3/MTmodel.pth'
...
if __name__ == "__main__":
    for k in range(8,9):
        ...
        train_data=torch.load("save3/train_set"+str(k)+".pth")
        train_label=torch.load("save3/train_label"+str(k)+".pth")
        test_data=torch.load("save3/test_set"+str(k)+".pth")
        test_label=torch.load("save3/test_label"+str(k)+".pth")
        val_data=torch.load("save3/val_set"+str(k)+".pth")
        val_label=torch.load("save3/val_label"+str(k)+".pth")

        train_set = TensorDataset(train_data, train_label)  # 训练数据装成TensorDataset
        val_set = TensorDataset(val_data, val_label)  # 验证数据装成TensorDataset
        test_set = TensorDataset(test_data, test_label)  # 测试数据装成TensorDataset
