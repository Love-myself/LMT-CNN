...
class TextCNN(nn.Module):
    def __init__(self, vec_dim, filter_num, sentence_max_size, label_size, kernel_list):
        ...
        # nn.Conv2d的输入是个四维的tensor，
        # 每一位分别代表batch_size、channel、length、width
...
def compute_classification(test_loader):
    ...
    result_name = open('result/MWLE_cnn.txt', "a")
...
if __name__ == '__main__':
    for i in range(10):
        print('epoch'+str(i))
        train_set = torch.load('save3/train_set'+str(i)+'.pth')  # 训练数据
        train_label = torch.load('save3/train_label'+str(i)+'.pth')  # 训练数据标签
        val_set = torch.load('save3/val_set'+str(i)+'.pth')  # 验证数据
        val_label = torch.load('save3/val_label'+str(i)+'.pth')  # 验证数据标签
        test_set = torch.load('save3/test_set'+str(i)+'.pth')   # 测试数据
        test_label = torch.load('save3/test_label'+str(i)+'.pth')  # 测试集标签
